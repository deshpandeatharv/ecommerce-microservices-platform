...
export const fetchCategories = () => async (dispatch) => {
    try {
        ...
        dispatch({ type: "CATEGORY_SUCCESS" }); // optional improvement
...
export const addUpdateUserAddress =
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      if (!addressId) {
        const response = await api.post("/addresses", sendData);
        data = response.data;
      } else {
        const response = await api.put(`/addresses/${addressId}`, sendData);
        data = response.data;
      }
      ...
      dispatch({ type: "BUTTON_LOADER" }); // toggle OFF if reducer supports toggle
...
export const deleteUserAddress =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      dispatch({ type: "BUTTON_LOADER" }); // IMPORTANT FIX
    }
  };
...
export const createUserCart =
  (sendCartItems) => async (dispatch) => {
    try {
      ...
      await api.post("/carts", sendCartItems);
...
export const stripePaymentConfirmation =
    (sendData, setErrorMessage, setLoading, toast, paymentMethod) =>
    async (dispatch, getState) => {
        try {
            setLoading(true);
...
export const analyticsAction = () => async (dispatch) => {
    try {
        ...
        const { data } = await api.get('/analytics/admin/app');
...
export const getOrdersForDashboard =
  (queryString, isAdmin) => async (dispatch) => {
    try {
      ...
      const endpoint = isAdmin ? "/orders" : "/orders/seller";

      const { data } = await api.get(`/api${endpoint}?${queryString}`);
...
export const updateOrderStatusFromDashboard =
  (orderId, orderStatus, toast, setLoader, isAdmin) => async (dispatch) => {
    try {
      ...
      const { data } = await api.put(`/api${endpoint}`, {
        status: orderStatus,
      });
...
export const addNewProductFromDashboard =
    (sendData, toast, reset, setLoader, setOpen, isAdmin) => async (dispatch) => {
        try {
            ...
            const endpoint = isAdmin
                ? "/api/products/admin/category/"
                : "/api/products/seller/category/";

            await api.post(
                `${endpoint}${sendData.categoryId}`,
                sendData
            );
...
export const getAllCategoriesDashboard =
...
export const createCategoryDashboardAction =
  (sendData, setOpen, reset, toast) => async (dispatch) => {
    try {
      ...
      await api.post("/api/categories", sendData);
...
export const updateCategoryDashboardAction =
  ...
  async (dispatch) => {
    try {
      ...
      await api.put(`/api/categories/${categoryID}`, sendData);
  ...
  (queryString) => async (dispatch, getState) => {
    try {
      ...
      dispatch({
        type: "IS_ERROR",
        payload:
          err?.response?.data?.message || "Failed to fetch sellers data",
      });
    }
  };
...
export const addNewDashboardSeller =
  (sendData, toast, reset, setOpen, setLoader) => async (dispatch) => {
    try {
      ...
      // IMPORTANT: ensure backend assigns ROLE_SELLER
      const payload = {
        ...sendData,
        role: "SELLER", // only if backend supports it
      };
